...
def create_folder_structure(video: Video, with_symlinks=True):
    ...
    for p in [main_path, Path(main_path, "poster")] + [Path(main_path, "gear" + str(pp)) for pp in range(1, 10)]:
...
def create_thumbnails(content: bytes, video_id: UUID):
    ...
    with Image.open(BytesIO(content)) as image:
        for poster in [Poster(1920, 1080, "fhd"), Poster(1280, 720, "hd"), Poster(854, 480, "sd")]:
            i = image.copy()
...
def update_symlinks(video: Video):
    ...
    short_video_path(video.url).symlink_to(main_path.resolve(), target_is_directory=True)
